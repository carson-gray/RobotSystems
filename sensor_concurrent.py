...
class Interpreter:
    ...
    def consumer_producer(self, sensor_bus_in, offset_bus_out, time_delay):
        while True:
            sensor_reading = sensor_bus_in.read()
            offset = self.process(sensor_reading)
            offset_bus_out.write(offset)
            time.sleep(time_delay)
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        ...
        e_sensor = executor.submit(sensor.producer,
                                   sensor_values_bus,
                                   sensor_delay)
        # interpreter consumes sensor readings, produces the car offset calculation
        e_interpreter = executor.submit(interpreter.consumer_producer,
                                        sensor_values_bus,
                                        interpreter_bus,
                                        interpreter_delay)
        # controller consumes car offset calculation, controls the car
        e_controller = executor.submit(controller.consumer,
                                       interpreter_bus,
                                       controller_delay)
